...
public class OntologySolver implements InferenceSolver {
      ...
      if (AnnotationUtils.areSameIgnoringValues(anno, Ontology)) {
        ...
        if (ontologyValues.length == 1) {
          SequenceSolver solver =
              new SequenceSolver(
                  ontologyValues[0], entry.getValue(), getSerializer(ontologyValues[0]));
  ...
  private List<SequenceSolution> solveInparallel(List<SequenceSolver> ontologySolvers)
    ...
    for (final SequenceSolver solver : ontologySolvers) {
      Callable<SequenceSolution> callable =
          new Callable<SequenceSolution>() {
            @Override
  ...
  protected InferenceSolution getMergedSolution(
      ProcessingEnvironment processingEnvironment, List<SequenceSolution> solutions) {
